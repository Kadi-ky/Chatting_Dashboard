...
import { MessageCircle, Sparkles, AlertCircle, Send } from 'lucide-react'
import { useChatMessages } from '../hooks/useChatMessages'
import { supabase } from '../lib/supabase'

// %% Mock Input Component %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function MockChatInput({ fanId, accountId, fanName }) {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke('chat-inbound', { body: payload })
  ...
  return (
    <form onSubmit={handleSend} className="pt-3 border-t border-white/10 flex items-center gap-2 mt-4">
      <input
        type="text"
        autoFocus
        value={msg}
        onChange={(e) => setMsg(e.target.value)}
        placeholder={`Message as ${fanName}...`}
      ...
      >
        <Send className="w-4 h-4" /> Simulate & Wait for Draft
      </button>
    </form>
  )
}
...
export default function TestingGroundTab({ creatorUuid }) {
    ...
    <div className="mt-3 sm:mt-5 grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-5">
      ...
      <div className="lg:col-span-1 bg-white/10 backdrop-blur-md border border-white/10 rounded-[20px] p-3 shadow-sm max-h-[75vh] overflow-y-auto">
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <h2 className="text-white font-semibold text-sm">Testing Ground</h2>
        ...
        {!loading && threads.length === 0 && (
          <div className="text-sm text-white/40 px-3 py-8 text-center">
            No conversations yet. Your simulated testing threads will appear here!
          </div>
        )}
      ...
      <div className="lg:col-span-2 bg-white/10 backdrop-blur-md border border-white/10 rounded-[20px] p-5 shadow-sm min-h-[60vh] flex flex-col">
        ...
        {selected && (
          <>
            ...
            <MockChatInput
              fanId={selected.fan_id}
              accountId={selected.onlyfans_account_id}
              fanName={selected.name}
            />
          </>
        )}
      </div>
